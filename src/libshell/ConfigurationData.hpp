...
struct DCSConfigurationData : ConfigurationDataBase
{
    ...
    {
        helpers::write_matrix_binary(basename+"_vertices.dat", vertexdata);
        helpers::write_matrix_binary(basename+"_edgedirs.dat", edgedata);
	    helpers::write_matrix(basename+"_edgedirs.txt", edgedata);
    }
    ...
    void readFromFile(const std::string & basename)
    {
        ...
        Eigen::MatrixXd vertexdata_in;
        Eigen::VectorXd edgedata_in;
        helpers::read_matrix_binary(basename+"_vertices.dat", vertexdata_in);
        helpers::read_matrix_binary(basename+"_edgedirs.dat", edgedata_in);

        // extract number of vertices and edges
        const int nVertices = vertexdata_in.rows();
        const int nEdges = edgedata_in.rows();

        // do initialization of base
        ConfigurationDataBase::init(3*nVertices + nEdges);
