...
class IOGeometry_Rectangle_Clamped : public Geometry
{
protected:
    enum class MESHTYPE {OBJ, OFF, VTP};
...
public:
    IOGeometry_Rectangle_Clamped(const std::string filename, const Real marginWidth, const bool clampedX, const bool clampedY):
    filename(filename),
    ...
    virtual void get(Eigen::MatrixXd & vertices, Eigen::MatrixXi & face2vertices, Eigen::MatrixXb & vertices_bc) const override
    {
        ...
        const Eigen::VectorXd min_vert = vertices.colwise().minCoeff();
        const Eigen::VectorXd max_vert = vertices.colwise().maxCoeff();
        const Real halfEdgeX = std::abs(max_vert(0) - min_vert(0))/2.0;
        const Real halfEdgeY = std::abs(max_vert(1) - min_vert(1))/2.0;
        ...
        for(int i=0;i<nV;++i)
        {
            ...
            if(vertices(i,0) < (CenterX - halfEdgeX*0.99 + marginWidth) and clampedX)
            {
            ...
            if(vertices(i,1) < (CenterY - halfEdgeY*0.99 + marginWidth) and clampedY)
            {
                vertices_bc(i,0) = true;
                vertices_bc(i,1) = true;
                vertices_bc(i,2) = true;
...
class Geometry_Dummy_With_BC : public PlateGeometry
{
    Eigen::MatrixXd my_vertices;
    Eigen::MatrixXi my_face2vertices;
    Eigen::MatrixXb my_vertices_bc;
    ...
    void getPlateGeometry(Eigen::MatrixXd & vertices, Eigen::MatrixXi & face2vertices, Eigen::MatrixXb & vertices_bc) const override
    {
        ...
        vertices_bc = my_vertices_bc;
        // initVertexBoundaries(vertices.rows(), vertices_bc);
    }

public:
    Geometry_Dummy_With_BC(const Eigen::Ref<const Eigen::MatrixXd> my_vertices_in, const Eigen::Ref<const Eigen::MatrixXi> my_face2vertices_in, const Eigen::Ref<const Eigen::MatrixXb> my_vertices_bc_in)
    {
        my_vertices = my_vertices_in;
        my_face2vertices = my_face2vertices_in;
        my_vertices_bc = my_vertices_bc_in;
    }
};
...
protected:
    ...
    virtual void getPlateGeometry(Eigen::MatrixXd & vertices, Eigen::MatrixXi & face2vertices, Eigen::MatrixXb & vertices_bc) const override
    {
        ...
        const int nX_tmp = (int)std::ceil(2*halfEdgeX/edgeLength);
        nX = (nX_tmp % 2 == 1) ? nX_tmp : nX_tmp+1;
        dX = 2*halfEdgeX / (nX-1);

        const int nY_tmp = (int)std::ceil(2*halfEdgeY/edgeLength);
        nY = (nY_tmp % 2 == 1) ? nY_tmp : nY_tmp+1;
        dY = 2*halfEdgeY / (nY-1);
        ...
        {
            // check if we are on the boundary
            if(std::abs(vertices(i,0) + halfEdgeX) < (1e-6*halfEdgeX + marginWidth) and clampedX)
            {
                vertices_bc(i,0) = true;
                vertices_bc(i,1) = true;
                vertices_bc(i,2) = true;
            ...
            if(std::abs(vertices(i,1) + halfEdgeY) < (1e-6*halfEdgeY + marginWidth) and clampedY)
            {
                vertices_bc(i,0) = true;
                vertices_bc(i,1) = true;
                vertices_bc(i,2) = true;
...
public:
    RectangularPlate_RightAngle_Clamped(const Real halfX, const Real halfY, const Real edgeLength, const Real marginWidth, const bool clampedX = true, const bool clampedY = true):
    halfEdgeX(halfX),
    halfEdgeY(halfY),
    edgeLength(edgeLength),
    marginWidth(marginWidth),
...
class
 CylinderTubeCurved : public ShellGeometry
{
protected:
...
class RectangularPlate_3clampvert : public PlateGeometry
{
protected:
    const Real halfEdgeX;
    const Real halfEdgeY;
    ...
    virtual void getPlateGeometry(Eigen::MatrixXd & vertices, Eigen::MatrixXi & face2vertices, Eigen::MatrixXb & vertices_bc) const override
    {
        ...
        for(int i=0;i<nV;++i)
        {
            ...
            if(0.99*halfEdgeX < vertices(i,0) && 1.01*halfEdgeX > vertices(i,0) && 0.99*halfEdgeY < vertices(i,1) && 1.01*halfEdgeY > vertices(i,1))
            {
                vertices_bc(i,0) = true;
                vertices_bc(i,1) = true;
                vertices_bc(i,2) = true;
...
public:
    RectangularPlate_3clampvert(const Real halfX, const Real halfY, const Real edgeLength, const bool closedX = false, const bool closedY = false):
    halfEdgeX(halfX),
    halfEdgeY(halfY),
    edgeLength(edgeLength),
    closedX(closedX),
...
class RectangularPlate_4clampvert : public PlateGeometry
{
protected:
    const Real halfEdgeX;
    const Real halfEdgeY;
...
//                const int v3_idx = (i+1)*nY + j;
        ...
        for(int i=0;i<nV;++i)
        {
            ...
            else if(0.99*halfEdgeX < vertices(i,0) && 1.01*halfEdgeX > vertices(i,0) && -0.99*halfEdgeY > vertices(i,1) && -1.01*halfEdgeY < vertices(i,1))
            {
                vertices_bc(i,2) = true;
            }
            else if(-0.99*halfEdgeX > vertices(i,0) && -1.01*halfEdgeX < vertices(i,0) && -0.99*halfEdgeY > vertices(i,1) && -1.01*halfEdgeY < vertices(i,1))
...
public:
    RectangularPlate_4clampvert(const Real halfX, const Real halfY, const Real edgeLength, const bool closedX = false, const bool closedY = false):
    halfEdgeX(halfX),
    halfEdgeY(halfY),
    edgeLength(edgeLength),
    closedX(closedX),
