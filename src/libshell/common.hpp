...
namespace helpers
{
    ...
    template<class Matrix>
    inline void write_matrix(const std::string & filename, const Matrix& matrix)
    {
    ...
    template<class Matrix>
    inline void read_matrix(const std::string & filename, Matrix& matrix, const Real size_1, const Real size_2)
    {
    ...
    inline void write_intintmap_binary(const std::string & filename, const std::map<int, int> & omap)
    {
        std::ofstream out(filename, std::ios::out | std::ios::binary | std::ios::trunc);
        const size_t mSize = omap.size();
        out.write((char*) (&mSize), sizeof(size_t));
        for(const auto & el : omap)
        {
            out.write((char*) (&el.first), sizeof(int));
            out.write((char*) (&el.second), sizeof(int));
        }
        out.close();
    }

    inline void read_intintmap_binary(const std::string & filename,  std::map<int, int> & omap)
    {
        omap.clear();
