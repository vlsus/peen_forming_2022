...
struct DCSBasicConfiguration
{
    ...
    {
        dcsdata.writeToFile(basename);
    }

    void readFromFile(const std::string & basename)
    {
        dcsdata.readFromFile(basename);
    }
};
...
struct DCSRestConfiguration : DCSBasicConfiguration
{
    ...
    void writeToFile(const std::string & basename) const
    {
        DCSBasicConfiguration::writeToFile(basename);
        helpers::write_vecmat_binary(basename+"_aform.dat", aform);
        helpers::write_vecmat_binary(basename+"_bform.dat", bform);
    }

    void readFromFile(const std::string & basename)
    {
        DCSBasicConfiguration::readFromFile(basename);
        helpers::read_vecmat_binary(basename+"_aform.dat", aform);
        helpers::read_vecmat_binary(basename+"_bform.dat", bform);
    }
};
...
struct DCSBilayerRestConfiguration : DCSBasicConfiguration
{
    ...
    tVecMat2d & getFFF_bot()
    {
        return aform_bot;
    }

    tVecMat2d & getFFF_top()
    {
        return aform_top;
    }
    ...
    {
        DCSBasicConfiguration::writeToFile(basename);
        helpers::write_vecmat_binary(basename+"_aform_bot.dat", aform_bot);
        helpers::write_vecmat_binary(basename+"_aform_top.dat", aform_top);
        helpers::write_vecmat_binary(basename+"_bform.dat", bform);
    }
    ...
    {
        DCSBasicConfiguration::readFromFile(basename);
        helpers::read_vecmat_binary(basename+"_aform_bot.dat", aform_bot);
        helpers::read_vecmat_binary(basename+"_aform_top.dat", aform_top);
        helpers::read_vecmat_binary(basename+"_bform.dat", bform);
    }
...
struct DCSDynamicCurrentConfiguration : DCSCurrentConfiguration
{
    ...
    void writeToFile(const std::string & basename) const
    {
        // this dumps vertices/edgedirs for mesh
        DCSCurrentConfiguration::writeToFile(basename);
        // this dumps velocities of them
        dyndcsdata.writeToFile(basename+"_dyndata");
    }

    void readFromFile(const std::string & basename)
    {
        // this dumps vertices/edgedirs for mesh
        DCSCurrentConfiguration::readFromFile(basename);
        // this dumps velocities of them
        dyndcsdata.readFromFile(basename+"_dyndata");
    }
